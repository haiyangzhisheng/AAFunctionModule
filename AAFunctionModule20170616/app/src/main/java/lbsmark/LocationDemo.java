...
import com.aaagame.module.activity.BaseFragmentActivity;
import com.aaagame.module.utils.AACom;
import com.aaagame.module.utils.AAToast;
 ...
 */
public class LocationDemo extends BaseFragmentActivity {
    ...
    private RelativeLayout llMLMain;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            llMLMain = (RelativeLayout) findViewById(R.id.llMLMain);
            // 地图初始化
        ...
        checkLocPer();
        // handler.postDelayed(myRunnable, 1000 * 1);
        // handler.postDelayed(myRunnable, 1000 * 2);
        // handler.postDelayed(myRunnable, 1000 * 3);
        // handler.postDelayed(myRunnable, 1000 * 4);
     ...
     * @return
     */
    public boolean checkLocPer() {
        try {
            if (ContextCompat.checkSelfPermission(myActivity,
                    Manifest.permission.ACCESS_COARSE_LOCATION) != PackageManager.PERMISSION_GRANTED || ContextCompat.checkSelfPermission(myActivity,
                    Manifest.permission.ACCESS_FINE_LOCATION) != PackageManager.PERMISSION_GRANTED) {
                // 申请WRITE_EXTERNAL_STORAGE权限
                ActivityCompat.requestPermissions(myActivity, new String[]{Manifest.permission.ACCESS_COARSE_LOCATION, Manifest.permission.ACCESS_FINE_LOCATION}, reqLoc);
                return false;
            }
        } catch (Exception e) {
            e.printStackTrace();
            ...
            switch (requestCode) {
                case reqLoc:
                    if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                        //等待地图初始化完毕和权限状态更新，两秒后执行
                        myTimeHandler.postDelayed(new Runnable() {
                            @Override
                            public void run() {
                                myTimeHandler.sendEmptyMessage(0);
                            }
                        }, 2000);
                    } else {
                        AAToast.toastShow(myActivity, "获取授权失败");
                    }
                    break;
                default:
            }
    ...
    Handler myTimeHandler = new Handler(new Callback() {
        @Override
    ...
    public void locate() {
        try {
            BaiduMapUtilByRacer.locateByBaiduMap(mContext, 2000, new BaiduMapUtilByRacer.LocateListener() {
                ...
                public void onLocateSucceed(LocationBean locationBean) {
                    try {
                        ...
                        System.out.println(AACom.getGson().toJson(mLocationBean) + "我的当前位置");
                        mMarker = BaiduMapUtilByRacer.showMarkerByResource(locationBean.getLatitude(),
                                locationBean.getLongitude(), R.drawable.bd_point, mBaiduMap, 0, true);

                        // 反Geo搜索
                        reverseGeoCode(new LatLng(mLocationBean.getLatitude(), mLocationBean.getLongitude()), false);
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                }
    ...
    public void getPoiByPoiSearch() {
        ...
        try {
            ...
            BaiduMapUtilByRacer.getPoiByPoiSearch(myCity, etMLCityPoi.getText().toString().trim(), 0,
                    new BaiduMapUtilByRacer.PoiSearchListener() {

                        @Override
                        public void onGetSucceed(List<LocationBean> locationList, PoiResult res) {

                            System.out.println("结果获取热点-----------------------" + AACom.getGson().toJson(locationList));
    ...
    public void reverseGeoCode(LatLng ll, final boolean isShowTextView) {
        BaiduMapUtilByRacer.getPoisByGeoCode(ll.latitude, ll.longitude, new BaiduMapUtilByRacer.GeoCodePoiListener() {
...
//            @Override
//            public void onClick(View v) {
//                if (aroundPoiList == null || aroundPoiList.isEmpty()) {
//                    Toast.makeText(myActivity, "无位置信息", Toast.LENGTH_SHORT).show();
//                    return;
//                }
//                PoiInfo poiInfo = aroundPoiList.get(0);
//                GpsData gpsData = new GpsData();
//                gpsData.Latitude = String.valueOf(poiInfo.location.latitude);
//                gpsData.Longitude = String.valueOf(poiInfo.location.longitude);
//                gpsData.City = poiInfo.city;
//                gpsData.Addr = poiInfo.address;
//                if (!TextUtils.isEmpty(mLocationBean.district)) {
//                    gpsData.Area = mLocationBean.district;
//                } else {
//                    gpsData.Area = "";
//                }
//                Intent intent = getIntent();
//                intent.putExtra("gpsdata", AACom.getGson().toJson(gpsData));
...
//        });
        lvAroundPoi.setOnItemClickListener(new OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> arg0, View arg1, int position, long arg3) {
                Toast.makeText(myActivity, "around显示", Toast.LENGTH_SHORT).show();
                // isCanUpdateMap = false;
                // BaiduMapUtilByRacer.moveToTarget(
                // aroundPoiList.get(position).location.latitude,
                // aroundPoiList.get(position).location.longitude,
                // mBaiduMap);
                ...
                // gpsData.Latitude = String.valueOf(poiInfo.location.latitude);
                // gpsData.Longitude =
                // String.valueOf(poiInfo.location.longitude);
                // gpsData.City = poiInfo.city;
                // if (position == 0) {
                // gpsData.Addr = poiInfo.address;
                // } else {
                // gpsData.Addr = poiInfo.address + "(" + poiInfo.name + ")";
                // }
                // Intent intent = getIntent();
                // intent.putExtra("gpsdata", AACom.getGson().toJson(gpsData));
                // setResult(RESULT_OK, intent);
        ...
        lvSearchPoi.setOnItemClickListener(new OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
                ...
                // 反Geo搜索
                reverseGeoCode(
                        new LatLng(searchPoiList.get(arg2).getLatitude(), searchPoiList.get(arg2).getLongitude()),
                        false);
    ...
    private void updatePoiListAdapter(List<PoiInfo> list, int index) {
        try {
            ...
            PoiInfo loc = new PoiInfo();
            LatLng latLng = new LatLng(mLocationBean.getLatitude(), mLocationBean.getLongitude());
            loc.location = latLng;
            loc.name = AACom.getColorHtmlText(this, R.color.red500, "[当前]" + mLocationBean.getStreet());

            loc.address = mLocationBean.getProvince() + mLocationBean.getCity() + mLocationBean.getDistrict()
                    + mLocationBean.getStreet();
            loc.city = mLocationBean.getCity();
            System.out.println(AACom.getGson().toJson(list));
            // list.clear();
            list.add(0, loc);
